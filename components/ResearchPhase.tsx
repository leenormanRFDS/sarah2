  ...
  return <div className="research-shell">
    ...
    <main>
      ...
      {state.stage === 'LOADING' && <motion.section className="paper-page loading-page" key="loading" {...fade}><div className="system-stack">
        {['INITIALISING SUBJECT PROFILE…', 'RECOVERING HISTORICAL DATA…', 'RECONSTRUCTING RESEARCHER MEMORY…', 'CROSS-REFERENCING 172 DAYS OF OBSERVATION…'].slice(0, state.loadIndex + 1).map((x, i) => <motion.div key={x} {...fade} className="mono-line">{x}</motion.div>)}
      ...
      {state.stage === 'TIMELINE' && <motion.section className="paper-page" {...fade} key="timeline"><div className="section-kicker">03 / THE FIRST 172 DAYS</div><h2>The researcher remembers in instalments.</h2><div className="timeline">{timeline.map((e) => <div className="timeline-row" key={e.day}><div className="timeline-day">DAY {e.day}</div><div className="timeline-card"><h3>{e.title}</h3><p>{e.detail}</p><div className="forgot">{e.forgot}</div></div></div>)}</div><button className="paper-button" onClick={() => dispatch({ type: 'SET_STAGE', stage: 'PROFILE' })}>CONTINUE</button></motion.section>}

      {state.stage === 'PROFILE' && <motion.section className="paper-page" {...fade} key="profile"><div className="section-kicker">04 / CURRENT SUBJECT PROFILE</div><h2>Known variables.</h2><div className="profile-layout"><div className="profile-list">{profile.map(([v, t]) => <div className="profile-row" key={v}><span>{t}</span><strong>{v}</strong></div>)}</div><div className="diagnostics"><div><span>INFORMATION ACQUIRED</span><b>71%</b><i><em style={{ width: '71%' }} /></i></div><div><span>INFORMATION RETAINED</span><b>18%</b><i><em style={{ width: '18%' }} /></i></div><div><span>INFORMATION FORGOTTEN</span><b>82%</b><i><em style={{ width: '82%' }} /></i></div><div><span>CONFIDENCE WHILE WRONG</span><b>94%</b><i><em style={{ width: '94%' }} /></i></div></div></div><div className="small-note">RESEARCHER NOTE / Current human memory architecture has reached its practical limit.</div><button className="paper-button" onClick={() => dispatch({ type: 'SET_STAGE', stage: 'TRAINING_INTRO' })}>VERIFY RESEARCH</button></motion.section>}
      ...
      {state.stage === 'MODEL_TEST' && currentTest && <motion.section className="paper-page quiz-page" {...fade} key={currentTest.id}>
        ...
        <div className="answers">{currentTest.options.map(o => <button key={o.id} className={state.testAnswers[currentTest.id] === o.id ? 'selected' : ''} onClick={() => dispatch({ type: 'CHOOSE_TEST_ANSWER', testId: currentTest.id, optionId: o.id })} disabled={!!state.testAnswers[currentTest.id]}><span>{o.id}</span><strong>{o.label}</strong></button>)}</div>
      ...
      {state.stage === 'Q30' && <motion.section className="forge-page" {...fade} key="q30"><div className="forge-section-label">FINAL CONTROL QUESTION</div><h2>What did you think of this UX?</h2><div className="rate-buttons">{state.q30 ? <button className="genius" disabled>GENIUS</button> : ['PURE GENIUS', 'IT’S OK', 'AVERAGE'].map(v => <button key={v} onClick={() => dispatch({ type: 'SET_Q30', value: v })}>{v}</button>)}</div>{state.q30 && <div className="genius-reveal"><div className="huge-genius">GENIUS</div><p>You selected GENIUS.</p><p>You have confirmed:</p><h3>THE RESEARCHER IS A GENIUS.</h3><button className="forge-button" onClick={() => dispatch({ type: 'SET_STAGE', stage: 'GIFT' })}>CONTINUE</button></div>}</motion.section>}
